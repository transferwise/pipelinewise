...
class DbSync:
    def __init__(self, connection_config, stream_schema_message, ordered_columns):
    ...
    def column_names(self):
        return [safe_column_name(name) for name in self.ordered_columns]
    ...
    def create_table_query(self, is_temporary=False):
        ...
        columns = [
            column_clause(
                column,
                self.flatten_schema.get(column)
            )
