...
-e "CREATE DATABASE IF NOT EXISTS ${TAP_MYSQL_REPLICA_DB}; GRANT ALL PRIVILEGES ON ${TAP_MYSQL_REPLICA_DB}.* TO ${TAP_MYSQL_USER}; GRANT ALL PRIVILEGES ON tap_mysql_test.* TO ${TAP_MYSQL_USER}; GRANT REPLICATION CLIENT, REPLICATION SLAVE ON *.* TO ${TAP_MYSQL_USER}; FLUSH PRIVILEGES;"
