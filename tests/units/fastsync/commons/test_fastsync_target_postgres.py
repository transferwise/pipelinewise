...
class TestFastSyncTargetPostgres(TestCase):
    ...
    def test_grant_select_on_table(self):
        ...
        assert self.postgres.executed_queries == ['GRANT SELECT ON test_schema."test_table" TO GROUP test_role']
        ...
        assert self.postgres.executed_queries == ['GRANT SELECT ON test_schema."full" TO GROUP test_role']
    ...
    def test_grant_usage_on_schema(self):
        """Validate if GRANT command generated correctly"""
        self.postgres.executed_queries = []
        self.postgres.grant_usage_on_schema(target_schema="test_schema", role="test_role")
        assert self.postgres.executed_queries == ["GRANT USAGE ON SCHEMA test_schema TO GROUP test_role"]

    def test_grant_select_on_schema(self):
        """Validate if GRANT command generated correctly"""
        self.postgres.executed_queries = []
        self.postgres.grant_select_on_schema(target_schema="test_schema", role="test_role")
        assert self.postgres.executed_queries == ["GRANT SELECT ON ALL TABLES IN SCHEMA test_schema TO GROUP test_role"]
    ...
    def test_swap_tables(self):
        ...
        self.postgres.executed_queries = []
        self.postgres.swap_tables(schema="test_schema", table_name="table with SPACE and UPPERCASE")
        assert self.postgres.executed_queries == [
            'DROP TABLE IF EXISTS test_schema."table with space and uppercase"',
            'ALTER TABLE test_schema."table with space and uppercase_temp" RENAME TO "table with space and uppercase"',
        ]
    ...
    def test_obfuscate_columns_case4(self):
        ...
        self.assertListEqual(
            ...
            [
                ...
                "(\"col_2\" ~ '[0-9]{3}\.[0-9]{3}');",  # noqa: W605
                'UPDATE "my_schema"."my_table_temp" SET '
                '"col_7" = CASE WHEN LENGTH("col_7") > 2 * 3 THEN '
                'CONCAT(SUBSTRING("col_7", 1, 3), REPEAT(\'*\', LENGTH("col_7")-(2 * 3)), '
                'SUBSTRING("col_7", LENGTH("col_7")-3+1, 3)) '
                'ELSE REPEAT(\'*\', LENGTH("col_7")) END WHERE ("col_1" = 30) AND '
                '("col_2" ~ \'[0-9]{3}\.[0-9]{3}\') AND ("col_4" IS NULL);',  # noqa: W605
