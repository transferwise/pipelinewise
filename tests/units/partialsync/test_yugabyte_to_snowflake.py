...
from pipelinewise.fastsync.partialsync import yugabyte_to_snowflake
...
class PartialSyncTestCase(TestCase):
    ...
    def test_yugabyte_partial_sync_rejects_removed_iceberg_create_before_connectors(self):
        ...
        with mock.patch.object(
            yugabyte_to_snowflake, 'FastSyncTapYugabyte'
        ) as source, mock.patch.object(
            yugabyte_to_snowflake, 'FastSyncTargetSnowflake'
        ) as target:
            with self.assertRaisesRegex(ValueError, 'iceberg_create'):
                yugabyte_to_snowflake.partial_sync_table(table, args)

        source.assert_not_called()
        target.assert_not_called()

    def test_yugabyte_main_rejects_removed_iceberg_create_before_pool_or_connectors(self):
        """Reject legacy routing before starting workers or connectors."""
        args = PartialSync2SFArgs(temp_test_dir='FOO_DIR')
        args.target = {'iceberg_create': False}

        with mock.patch.object(
            yugabyte_to_snowflake.utils,
            'parse_args_for_partial_sync',
            return_value=args,
        ), mock.patch.object(
            yugabyte_to_snowflake.common_utils, 'get_pool_size'
        ) as get_pool_size, mock.patch.object(
            yugabyte_to_snowflake, 'FastSyncTapYugabyte'
        ) as source, mock.patch.object(
            yugabyte_to_snowflake, 'FastSyncTargetSnowflake'
        ) as target, mock.patch.object(
            yugabyte_to_snowflake.multiprocessing, 'Pool'
        ) as pool:
            with self.assertRaisesRegex(ValueError, 'iceberg_create'):
                yugabyte_to_snowflake.main_impl()
        ...
        for table_format in (None, 'native'):
            with self.subTest(table_format=table_format):
                ...
                with mock.patch.object(
                    ...
                    result = yugabyte_to_snowflake.partial_sync_table(table, args)

                self.assertIn('found managed_iceberg_v3', result)
                source.assert_not_called()
                self.assertEqual(target.return_value.method_calls, [])

    def test_yugabyte_to_snowflake_partial_sync_table_if_exception_happens(self):
        ...
        with mock.patch(
            'pipelinewise.fastsync.partialsync.yugabyte_to_snowflake.'
            'FastSyncTapYugabyte.open_connection'
        ) as mocked_yugabyte_connection, mock.patch.object(
            yugabyte_to_snowflake.iceberg_routes,
            'require_native_target_format',
        ):
            mocked_yugabyte_connection.side_effect = Exception(exception_message)
            actual_return = yugabyte_to_snowflake.partial_sync_table(test_table, args)
    ...
    @mock.patch('pipelinewise.fastsync.partialsync.yugabyte_to_snowflake.FastSyncTapYugabyte')
    @mock.patch('pipelinewise.fastsync.partialsync.yugabyte_to_snowflake.FastSyncTargetSnowflake')
    @mock.patch.object(
        yugabyte_to_snowflake.iceberg_routes,
        'require_native_target_format',
    )
    def test_yugabyte_partial_sync_failure_state_semantics(
        self,
        ...
        for failure_method, error_message in (
            ...
            with self.subTest(failure_method=failure_method), TemporaryDirectory() as temp_directory:
                ...
                mocked_fastsyncyb.return_value = source
                ...
                result = yugabyte_to_snowflake.partial_sync_table(test_table, args)
    ...
    def test_yugabyte_empty_unbounded_sync_publishes_and_advances_state(
        self,
        _mocked_native_format,
        mocked_fastsync_sf,
        mocked_fastsyncyb,
        ...
        """An empty YugabyteDB export still publishes its range and bookmark."""
        ...
        source = mocked_fastsyncyb.return_value
    ...
    def test_export_source_table_data(self):
        ...
        with TemporaryDirectory() as temp_test_dir:
            ...
            tap_id = 'tap_id_foo'
            with mock.patch(
                    'pipelinewise.fastsync.commons.tap_yugabyte.FastSyncTapYugabyte.copy_table') as mocked_copy_table:
                mocked_copy_table.side_effect = mocked_copy_table_method

                test_fast_sync = FastSyncTapYugabyte({}, {})
    ...
    @mock.patch('pipelinewise.fastsync.partialsync.yugabyte_to_snowflake.multiprocessing.Pool')
    def test_running_partial_sync_yugabyte_to_snowflake(self, mocked_pool):
        """Test the whole partial_sync_yugabyte_to_snowflake module works as expected"""
        test_table = {}
        expected_args = None
        ...
        with TemporaryDirectory() as temp_directory:
            ...
            for end_value in test_end_values:
                with self.subTest(endvalue=end_value):
                    arguments = {
                        'tap': f'{self.config_dir}/target_snowflake/tap_yugabyte/config.json',
                    ...
                    with self.assertLogs('pipelinewise') as actual_logs:
                        ...
                        with mock.patch('sys.argv', argv_list):
                            expected_args = parse_args_for_partial_sync(yugabyte_to_snowflake.REQUIRED_CONFIG_KEYS)
                            yugabyte_to_snowflake.main()
    ...
    def test_yugabyte_to_snowflake_partial_sync_table(self,
                                                        _mocked_native_format,
                                                        mocked_fastsync_sf, mocked_bookmark, mocked_fastsyncyb,
                                                        mocked_save_state, mocked_upload_to_s3, mocked_load_into_sf):
        """Test yugabyte to sf partial sync table"""
        ...
        for end_value in test_end_values:
            ...
            with TemporaryDirectory() as temp_directory:
                ...
                mocked_export_data = mocked_fastsyncyb.return_value.export_source_table_data
                mocked_fastsyncyb.return_value.map_column_types_to_target.return_value = maped_column_types_to_target
                mocked_export_data.side_effect = export_data_to_file

                actual_return = yugabyte_to_snowflake.partial_sync_table(test_table, args)
                self.assertIs(actual_return, True)

                mocked_fastsyncyb.assert_called_once_with(
                    args.tap, yugabyte_to_snowflake.tap_type_to_target_type
                )
                ...
                mocked_fastsyncyb.return_value.close_connection.assert_called_once_with()
