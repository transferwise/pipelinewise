...
class PartialSyncTestCase(TestCase):
    ...
    def test_export_source_table_data(self):
        ...
        with TemporaryDirectory() as temp_test_dir:
            ...
            tap_id = "tap_id_foo"
            with mock.patch(
                "pipelinewise.fastsync.commons.tap_postgres.FastSyncTapPostgres.copy_table"
            ) as mocked_copy_table:
    ...
    def test_postgres_to_snowflake_partial_sync_table(
        self,
        mocked_fastsync_sf,
        mocked_bookmark,
        mocked_fastsyncpg,
        ...
        for end_value in test_end_values:
            ...
            with TemporaryDirectory() as temp_directory:
                ...
                maped_column_types_to_target = {"columns": ["foo type1", "bar type2"], "primary_key": "foo_primary"}
