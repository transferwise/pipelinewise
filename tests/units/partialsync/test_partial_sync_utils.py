...
class PartialSyncUtilsTestCase(TestCase):
    ...
    def test_load_into_snowflake_hard_delete(self):
        ...
        target = {"sf_object": snowflake, "schema": "FOO_SCHEMA", "table": "FOO_TABLE", "temp": "FOO_TEMP"}
        args = PartialSync2SFArgs(temp_test_dir="temp_test_dir", start_value="20", end_value="30")
        columns_diff = {"added_columns": ["FOO_ADDED_COLUMN"], "source_columns": {"FOO_SOURCE_COLUMN": "FOO_TYPE"}}
        primary_keys = ["FOO_PRIMARY"]
        s3_key_pattern = "FOO_PATTERN"
        size_bytes = 3
        where_clause_sql = "test"
        load_into_snowflake(target, args, columns_diff, primary_keys, s3_key_pattern, size_bytes, where_clause_sql)
    ...
    def test_load_into_snowflake_soft_delete(self):
        ...
        snowflake = mock.MagicMock()
        target = {"sf_object": snowflake, "schema": "FOO_SCHEMA", "table": "FOO_TABLE", "temp": "FOO_TEMP"}
        args = PartialSync2SFArgs(temp_test_dir="temp_test_dir", start_value="20", end_value="30", hard_delete=False)
        columns_diff = {"added_columns": ["FOO_ADDED_COLUMN"], "source_columns": {"FOO_SOURCE_COLUMN": "FOO_TYPE"}}
        primary_keys = ["FOO_PRIMARY"]
        s3_key_pattern = "FOO_PATTERN"
        size_bytes = 3
    ...
    def test_find_diff_columns(self):
        ...
        sample_target_sf = {"sf_object": mocked_snowflake, "schema": schema, "table": table}

        expected_output = {
            "added_columns": {'"FOO_COLUMN_0"': "NUMBER", '"FOO_COLUMN_5"': "VARCHAR"},
            "removed_columns": {
                '"FOO_COLUMN_2"': "TEXT",
                '"FOO_COLUMN_4"': "NUMBER",
    ...
    def test_validate_dynamic_boindary_value_raise_exception_if_invalid_value(self):
        ...
        invalid_values = (
            ...
            "<D>foo;bar",
            "<D>delete from foo;",
            "<D>select * from foo;DELETE foo;",
            "<D>update foo set bar=baz",
            "<D>INSERT into foo (bar) values (baz);",
            "<D>foo",
        )
    ...
    def test_validate_dynamic_value_raise_excp_if_return_more_than_one_column_or_row(self):
        ...
        query_object = mock.MagicMock()
        query_returns = ([["foo", "bar"]], [{"foo": 1, "bar": 2}], [["foo"], ["bar"]], [{"foo": 1}, {"bar": 2}])
