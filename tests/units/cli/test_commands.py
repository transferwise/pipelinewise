...
class TestCommands:
    ...
    def test_build_tap_command(self):
        ...
        command = commands.build_tap_command(tap=tap, profiling_mode=False, profiling_dir=None)

        assert command == "/bin/tap_mysql.py --config .ppw/config.json --catalog .ppw/properties.json "

        # profiling is enabled
        command = commands.build_tap_command(tap=tap, profiling_mode=True, profiling_dir="./profiling")

        assert (
            command == "/bin/python3 -m cProfile -o ./profiling/tap_my_tap_mysql.pstat /bin/tap_mysql.py "
        ...
        # profiling is not enabled
        command = commands.build_tap_command(tap=tap, profiling_dir=None, profiling_mode=False)
        ...
        assert (
            command == f"/bin/tap_mysql.py --config .ppw/config.json --catalog .ppw/properties.json "
            f"--state {state_mock}"
        )
        ...
        assert (
            command == f"/bin/python3 -m cProfile -o ./profiling/tap_my_tap_mysql.pstat /bin/tap_mysql.py "
    ...
    def test_build_target_command(self):
        ...
        # profiling is not enabled
        command = commands.build_target_command(target=target, profiling_mode=False, profiling_dir=None)

        assert command == "/bin/target_postgres.py --config .ppw/config.json"

        # profiling is enabled
        command = commands.build_target_command(target=target, profiling_mode=True, profiling_dir="./profiling")
        assert (
            command == "/bin/python -m cProfile -o ./profiling/target_my_target.pstat /bin/target_postgres.py"
            " --config .ppw/config.json"
        )
    ...
    def test_build_transform_command(self):
        ...
        # profiling disabled
        command = commands.build_transformation_command(transform=transform, profiling_mode=False, profiling_dir=None)
        ...
        assert (
            command == f"/bin/python -m cProfile -o ./profiling/transformation_my_tap_my_target.pstat "
    ...
    def test_build_stream_buffer_command(self):
        ...
        assert (
            commands.build_stream_buffer_command(buffer_size=100, stream_buffer_bin="dummy_buffer")
            == "dummy_buffer -m 100M"
        )

        # Should log mbuffer status to log file with .running extension
        assert (
            commands.build_stream_buffer_command(buffer_size=100, log_file="stream_buffer.log")
    ...
    def test_build_singer_command(self):
        ...
        assert (
            command == f"/bin/tap_mysql.py --config .ppw/config.json --properties .ppw/properties.json "
            f"--state {state_mock}"
            f" | /bin/transform_field.py --config {transform_config}"
        ...
        assert (
            command == f"/bin/python -m cProfile -o ./profiling/tap_my_tap.pstat /bin/tap_mysql.py "
        ...
        assert (
            command == "/bin/tap_mysql.py --config .ppw/config.json --properties .ppw/properties.json "
            " | /bin/target_postgres.py --config .ppw/config.json"
        )
        ...
        assert (
            command == "/bin/python -m cProfile -o ./profiling/tap_my_tap.pstat /bin/tap_mysql.py "
