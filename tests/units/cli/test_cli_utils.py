...
class TestUtils:
    ...
    def test_json_loader(self):
        ...
        assert cli.utils.load_json(f"{os.path.dirname(__file__)}/resources/example.json") == {
            "glossary": {
    ...
    def test_yaml_loader(self):
        ...
        with pytest.raises(Exception):
            cli.utils.load_yaml(
                "{}/resources/example.yml".format(os.path.dirname(__file__)), "invalid-secret-file-path"
            )

        # Loading valid YAML file with no vault encryption
        assert cli.utils.load_yaml("{}/resources/example.yml".format(os.path.dirname(__file__))) == [
            "Apple",
            "Orange",
            "Strawberry",
            "Mango",
        ...
        assert cli.utils.load_yaml("{}/resources/example-with-jinja-env-var.yml".format(os.path.dirname(__file__))) == {
            "app": "my-app",
            "secret": app_secret,
    ...
    def test_sample_file_path(self):
        ...
        for sample in cli.utils.get_sample_file_paths():
            ...
            assert (
                re.match(".*config.yml$", sample)
                or re.match(".*(tap|target)_.*.yml.sample$", sample)
                or re.match(".*README.md$", sample)
            )

    def test_extract_log_attributes(self):
        """Log files must match to certain pattern with embedded attributes in the file name"""
        assert cli.utils.extract_log_attributes("snowflake-fx-20190508_000038.singer.log.success") == {
        ...
        assert cli.utils.extract_log_attributes("snowflake-fx-20190508_231238.fastsync.log.running") == {
    ...
    def test_delete_keys(self):
        ...
        assert cli.utils.delete_empty_keys({"foo": "bar", "foo2": None, "foo3": None, "foo4": "bar4"}) == {
            "foo": "bar",
            "foo4": "bar4",
        }
        ...
        assert cli.utils.delete_keys_from_dict(
            [{"foo": "bar", "foo2": "bar2"}, {"foo3": {"nested_foo": "nested_bar", "nested_foo2": "nested_bar2"}}],
            ["foo2", "nested_foo"],
    ...
    def test_tap_properties(self):
        ...
        assert (
            cli.utils.get_tap_stream_id(tap_mysql, "dummy_db", "dummy_schema", "dummy_table")
            == "dummy_schema-dummy_table"
        )

        # MySQL stream_name should be formatted as {{schema_name}}-{{table_name}}
        assert (
            cli.utils.get_tap_stream_name(tap_mysql, "dummy_db", "dummy_schema", "dummy_table")
            == "dummy_schema-dummy_table"
        )
        ...
        assert cli.utils.get_tap_extra_config_keys(tap_snowflake) == {"tables": "SCHEMA_1.TABLE_ONE,SCHEMA_1.TABLE_TWO"}
    ...
    def test_get_tap_target_names(self):
        ...
        tap_names, target_names = cli.utils.get_tap_target_names(
            f"{os.path.dirname(__file__)}/resources/test_tap_target_names"
        )
    ...
    def test_create_temp_file(self):
        ...
        temp_file = cli.utils.create_temp_file(
            dir="./temp_dir_to_create_automatically/deep_temp_dir",
            suffix=".json",
