...
class TestTargetPostgres:
    ...
    def test_replicate_mariadb_to_pg(self, tap_mariadb_id=TAP_MARIADB_ID):
        ...
        self.run_query_tap_mysql("UPDATE address SET street_number = 9999, date_updated = NOW() WHERE address_id = 1")
        #  FULL_TABLE
        ...
        result = self.run_query_target_postgres(
            'SELECT count(1) FROM ppw_e2e_tap_mysql."edgydata" where "group" is not null and "case" = \'B\';'
        )[0][0]
    ...
    def test_replicate_mariadb_replica_to_pg(self):
        """Replicate data from MariaDB to Postgres DWH"""
        # 1. Run tap first time - both fastsync and a singer should be triggered
        assertions.assert_run_tap_success(TAP_MARIADB_REPLICA_ID, TARGET_ID, ["fastsync", "singer"], profiling=True)
        assertions.assert_row_counts_equal(self.run_query_tap_mysql_2, self.run_query_target_postgres)
        assertions.assert_all_columns_exist(
    ...
    def test_replicate_pg_to_pg(self):
        ...
        result = self.run_query_target_postgres(
            "SELECT updated_at FROM ppw_e2e_tap_postgres.\"table_with_space and uppercase\" where cvarchar='H';"
        )[0][0]

        assert result == datetime(9999, 12, 31, 23, 59, 59, 999000)

        result = self.run_query_target_postgres(
            "SELECT updated_at FROM ppw_e2e_tap_postgres.\"table_with_space and uppercase\" where cvarchar='I';"
        )[0][0]
    ...
    def test_replicate_s3_to_pg(self):
        ...
        assertions.assert_run_tap_success(TAP_S3_CSV_ID, TARGET_ID, ["singer"])
