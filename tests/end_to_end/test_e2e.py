...
class TestE2E:
    ...
    def run_command(self, command):
        ...
        proctuple = proc.communicate()
        returncode = proc.returncode
        stdout = proctuple[0].decode('utf-8')
        stderr = proctuple[1].decode('utf-8')
    ...
    def assert_command_success(self, returncode, stdout, stderr, log_path=None):
    ...
    def test_import_project(self):
        ...
        [returncode, stdout, stderr] = self.run_command('pipelinewise import_config --dir {}'.format(self.project_dir))
        self.assert_command_success(returncode, stdout, stderr)
    ...
    def test_replicate_mariadb_to_pg(self):
        ...
        [returncode, stdout,
         stderr] = self.run_command('pipelinewise run_tap --tap mariadb_source --target postgres_dwh')
    ...
    def test_replicate_pg_to_pg(self):
        ...
        [returncode, stdout,
         stderr] = self.run_command('pipelinewise run_tap --tap postgres_source --target postgres_dwh')
        log_file = self.find_run_tap_log_file(stdout)
        self.assert_command_success(returncode, stdout, stderr, log_file)

    @pytest.mark.dependency(depends=['import_config'])
    def test_replicate_pg_to_snowflake(self):
