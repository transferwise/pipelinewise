...
class E2EEnv:
    ...
    def _load_env(self):
            ...
            "TARGET_SNOWFLAKE": {
                ...
                "vars": {
                    ...
                    "AWS_SECRET_ACCESS_KEY": {
                        "value": os.environ.get("TARGET_SNOWFLAKE_AWS_SECRET_ACCESS_KEY"),
                        "optional": True,
                    },
                    "SESSION_TOKEN": {
        ...
        self.env["TAP_MONGODB"]["is_configured"] = self._is_env_connector_configured("TAP_MONGODB")
        self.env["TARGET_POSTGRES"]["is_configured"] = self._is_env_connector_configured("TARGET_POSTGRES")
        self.env["PIPELINEWISE_BACKEND"]["is_configured"] = self._is_env_connector_configured("PIPELINEWISE_BACKEND")
    ...
    def setup_tap_s3_csv(self):
        ...
        s3 = boto3.client(
            ...
            aws_secret_access_key=self.get_conn_env_var("TAP_S3_CSV", "AWS_SECRET_ACCESS_KEY"),
        )
    ...
    def setup_target_postgres(self):
        ...
        self.run_query_target_postgres("DROP SCHEMA IF EXISTS ppw_e2e_tap_mysql_2 CASCADE")
    ...
    def setup_target_snowflake(self):
        ...
        if self.env["TARGET_SNOWFLAKE"]["is_configured"]:
            ...
            self.run_query_target_snowflake(f"DROP SCHEMA IF EXISTS ppw_e2e_tap_mysql{self.sf_schema_postfix} CASCADE")
            self.run_query_target_snowflake(
                f"DROP SCHEMA IF EXISTS ppw_e2e_tap_mysql_2{self.sf_schema_postfix} CASCADE"
            )
            self.run_query_target_snowflake(f"DROP SCHEMA IF EXISTS ppw_e2e_tap_s3_csv{self.sf_schema_postfix} CASCADE")
            self.run_query_target_snowflake(
    ...
    def add_column_into_source(self, tap_type, table, new_column):
        ...
        run_query_method(f"ALTER TABLE {table} ADD {new_column['name']} int")
        run_query_method(f"UPDATE {table} set {new_column['name']}={new_column['value']} where 1=1")

    def delete_record_from_source(self, tap_type, table, where_clause):
        """Delete a record from the source"""
        run_query_method = getattr(self, f"run_query_tap_{tap_type}")
        run_query_method(f"DELETE FROM {table} {where_clause}")
