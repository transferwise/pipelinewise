...
def assert_resync_tables_success(tap, target, profiling=False):
    ...
    if profiling:
        assert_profiling_stats_files_created(stdout, "fast_sync", ["fastsync"], tap, target)
...
def assert_partial_sync_table_with_target_additional_columns(tap_parameters, additional_column, start_value, end_value):
    ...
    tap_parameters["env"].add_column_into_target_sf(
        tap_type=tap_parameters["tap_type"], table=tap_parameters["table"], new_column=additional_column
    )
...
def assert_partial_sync_table_with_source_additional_columns(tap_parameters, additional_column, start_value, end_value):
...
def _map_tap_to_target_functions(
    ...
    f_map = {
        ...
        "run_query_tap_postgres": {
            ...
            "target_schemas": [
                f"ppw_e2e_tap_postgres{schema_postfix}",
...
def assert_row_counts_equal(
    ...
    if "target_sql_safe_name_fn" in funcs:
        row_counts_in_source = [
            (funcs["target_sql_safe_name_fn"](table), row_count) for (table, row_count) in row_counts_in_source
        ]
...
def assert_all_columns_exist(
    ...
    source_table_columns_map = {
        table[0].lower(): _cols_list_to_dict(table[1].lower().split(";")) for table in source_table_cols_raw
    }
    target_table_columns_map = {
        table[0].lower(): _cols_list_to_dict(table[1].lower().split(";")) for table in target_table_cols_raw
    }
    ...
    for source_table_name, source_table_columns in source_table_columns_map.items():
        ...
        for source_column_name, source_column_type_info in source_table_columns.items():
            ...
            if source_column_name not in target_table_columns:
                raise Exception(f'"{source_column_name}" column not found in target table "{source_table_name}"')
...
def _get_command_for_partial_sync(tap_parameters, start_value, end_value=None):
    ...
    command = (
        f"pipelinewise partial_sync_table --tap {tap_parameters['tap']} --target {tap_parameters['target']}"
