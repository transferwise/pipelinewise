...
class TestMySqlToSnowflakeDataDiff:
    ...
    def setup_method(self):
        ...
        self.full_check_name = (
            f'{TARGET_ID}/{TAP_ID}/{self.source_database}/{SOURCE_TABLE}'
        )
        self.target_schema = (
            f'ppw_e2e_data_diff_mysql_sf{self.e2e.sf_schema_postfix}'.upper()
        )
    ...
    def test_dd_reconciles_mysql_source_against_snowflake_target(self):
        ...
        target_count = self.run_target_query(
            f'SELECT COUNT(*) FROM {self.target_schema}.{SOURCE_TABLE.upper()}'
        )[0][0]
        assert target_count == source_count

        self._run_success(
            f'pipelinewise run_data_diff_checks --target {TARGET_ID} --tap {TAP_ID}'
        )
    ...
    def test_dd_detects_a_snowflake_target_that_diverged(self):
        ...
        self.run_target_query(
            f'UPDATE {self.target_schema}.{SOURCE_TABLE.upper()} '
            "SET WEIGHT_UNIT_NAME = WEIGHT_UNIT_NAME || '-diverged'"
        )
