...
class TestMySqlToPostgresDataDiff:
    ...
    def setup_method(self):
        ...
        self.full_check_name = f"{TARGET_ID}/{TAP_ID}/{self.e2e.get_conn_env_var('TAP_MYSQL', 'DB')}/{SOURCE_TABLE}"
    ...
    def test_dd_reconciles_mysql_source_against_postgres_target(self):
        ...
        assert self.run_target_query(f"SELECT COUNT(*) FROM {TARGET_SCHEMA}.{SOURCE_TABLE}")[0][0] == source_count

        stdout = self._run_success(f"pipelinewise run_data_diff_checks --target {TARGET_ID} --tap {TAP_ID}")
