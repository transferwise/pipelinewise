...
class TestDefinedPartialSyncMariaDBToSF(TapMariaDB):
    ...
    def _manipulate_target_tables(self):
        ...
        self.e2e_env.run_query_target_snowflake(
            f"DELETE FROM ppw_e2e_tap_mysql{self.e2e_env.sf_schema_postfix}.address WHERE address_id=500"
        )
        self.e2e_env.run_query_target_snowflake(
        ...
        self.e2e_env.run_query_target_snowflake(
            f"DELETE FROM ppw_e2e_tap_mysql{self.e2e_env.sf_schema_postfix}.weight_unit WHERE weight_unit_id=25"
        )

        self.e2e_env.run_query_target_snowflake(
            f"DELETE FROM ppw_e2e_tap_mysql{self.e2e_env.sf_schema_postfix}.customers WHERE id=15"
        )
    ...
    def test_defined_partial_sync_mariadb_to_sf(self):
        ...
        assertions.assert_record_count_in_sf(
            self.e2e_env, self.tap_type, "weight_unit", expected_records, f"WHERE weight_unit_id >= {from_value_weight}"
        )
        ...
        assertions.assert_record_count_in_sf(
            ...
            f"WHERE address_id >= {from_value_address}",
        )
        ...
        assertions.assert_record_count_in_sf(
            ...
            self.tap_type,
            "address",
            expected_records_less_than_from_value,
            f"WHERE address_id < {from_value_address}",
        )
