...
class TestReplicateMariaDBToSF(TapMariaDB):
    ...
    def test_replicate_mariadb_to_sf(self):
        ...
        result = self.e2e_env.run_query_target_snowflake(
            f'SELECT "C_VARCHAR" FROM ppw_e2e_tap_mysql{self.e2e_env.sf_schema_postfix}.edgydata WHERE "ORDER" = 11'
        )
        ...
        #  LOG_BASED
        self.e2e_env.run_query_tap_mysql("UPDATE weight_unit SET isactive = 0 WHERE weight_unit_id IN (2, 3, 4)")
        self.e2e_env.run_query_tap_mysql(
        ...
        self.e2e_env.run_query_tap_mysql(
            "UPDATE address SET street_number = 9999, date_updated = NOW() WHERE address_id = 1"
        )
        #  FULL_TABLE
