...
class TestReplicatePGToSFWithArchiveLoadFiles(TapPostgres):
    ...
    def delete_dangling_files_from_archive(self):
        ...
        for file_in_archive in files_in_s3_archive:
            self.s3_client.delete_object(Bucket=self.s3_bucket, Key=(file_in_archive["Key"]))
    ...
    def test_replicate_pg_to_sf_with_archive_load_files(self):
        ...
        for (
            ...
            archive_metadata = self.s3_client.head_object(Bucket=self.s3_bucket, Key=(files_in_s3_archive[0]["Key"]))[
                "Metadata"
            ]
            ...
            with tempfile.NamedTemporaryFile() as tmpfile:
                with open(tmpfile.name, "wb") as tmpf:
                    self.s3_client.download_fileobj(self.s3_bucket, files_in_s3_archive[0]["Key"], tmpf)
                with gzip.open(tmpfile, "rt") as gzipfile:
                    rows_in_csv = len(gzipfile.readlines())

            rows_in_table = self.e2e_env.run_query_tap_postgres(f"SELECT COUNT(1) FROM {schema_table}")[0][0]
