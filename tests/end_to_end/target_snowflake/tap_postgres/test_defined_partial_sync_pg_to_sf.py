...
class TestDefinedPartialSyncPGToSF(TapPostgres):
    ...
    def _manipulate_target_tables(self):
        self.e2e_env.run_query_target_snowflake(
            f"INSERT INTO ppw_e2e_tap_postgres{self.e2e_env.sf_schema_postfix}.\"ORDER\" (id, CVARCHAR) VALUES (1, 'A')"
        )

        self.e2e_env.run_query_target_snowflake(
            f'DELETE FROM ppw_e2e_tap_postgres{self.e2e_env.sf_schema_postfix}."ORDER" WHERE id=6'
        )
        self.e2e_env.run_query_target_snowflake(
            f"INSERT INTO ppw_e2e_tap_postgres{self.e2e_env.sf_schema_postfix}.CITY (id, name) VALUES (1, 'foo')"
        )

        self.e2e_env.run_query_target_snowflake(
            f"DELETE FROM ppw_e2e_tap_postgres{self.e2e_env.sf_schema_postfix}.CITY WHERE id=500"
        )

        self.e2e_env.run_query_target_snowflake(
            f"DELETE FROM ppw_e2e_tap_postgres{self.e2e_env.sf_schema_postfix}.customers WHERE id=15"
        )
    ...
    def test_defined_partial_sync_pg_to_sf(self):
        ...
        assertions.assert_record_count_in_sf(
            ...
            self.tap_type,
            "city",
            expected_records_greater_than_from_value,
            f"WHERE id >= {from_value_city}",
        )
