import os

from tests.end_to_end.target_snowflake.tap_yugabyte import TapYugabyte
from tests.end_to_end.helpers import tasks
from tests.end_to_end.target_snowflake import TEST_PROJECTS_DIR_PATH


TAP_ID = 'yugabyte_to_sf'
TARGET_ID = 'snowflake'


def _create_ppw_config_file(table_mb):
    with open(f'{TEST_PROJECTS_DIR_PATH}/config.yml', 'w', encoding='utf-8') as config_file:
        config_file.write('allowed_resync_max_size:\n')
        config_file.write(f'  table_mb: {table_mb}\n')

    return_code, stdout, stderr = tasks.run_command(
        f'pipelinewise import_config --dir {TEST_PROJECTS_DIR_PATH} '
        f'--taps {TAP_ID}'
    )

    assert return_code == 0, f'import_config failed\n{stdout}\n{stderr}'


class TestResyncYugabyteToSF(TapYugabyte):
    """Test Resync YugabyteDB to Snowflake."""
    def setUp(self, *args, **kwargs):  # pylint: disable = unused-argument
        super().setUp(tap_id=TAP_ID, target_id=TARGET_ID)

    def tearDown(self):
        try:
            os.remove(f'{TEST_PROJECTS_DIR_PATH}/config.yml')
        except OSError:
            pass
        super().tearDown()

    def test_resync_yugabyte_to_sf_if_table_size_greater_than_limit(self):
        """test resync yugabyte to SF returns error 1 if table size is greater than the limit"""

        a_small_number = 0.001  # Mb
        _create_ppw_config_file(table_mb=a_small_number)

        command = f'pipelinewise fast_sync --tap {TAP_ID} --target {TARGET_ID}'

        [return_code, _, _] = tasks.run_command(command)

        assert return_code == 1

    def test_resync_yugabyte_to_sf_if_table_size_less_than_limit(self):
        """test resync yugabyte to SF returns error if table size is less than the limit"""
        a_big_number = 1000  # Mb
        _create_ppw_config_file(table_mb=a_big_number)

        command = f'pipelinewise fast_sync --tap {TAP_ID} --target {TARGET_ID}'
        [return_code, _, _] = tasks.run_command(command)

        assert return_code == 0

    def test_resync_yugabyte_to_sf_if_table_size_greater_than_limit_and_force(self):
        """test resync yugabyte to SF returns error if table size is greater than the limit and --force is used"""
        a_small_number = 0.001  # Mb
        _create_ppw_config_file(table_mb=a_small_number)

        command = f'pipelinewise fast_sync --tap {TAP_ID} --target {TARGET_ID} --force'

        [return_code, _, _] = tasks.run_command(command)

        assert return_code == 0

    def test_run_tap_yugabyte_to_sf_if_size_greater_than_limit(self):
        """test run_tap yugabyte to sf if table size is greater than the limit"""
        a_small_number = 0.001  # Mb
        _create_ppw_config_file(table_mb=a_small_number)

        command = f'pipelinewise run_tap --tap {TAP_ID} --target {TARGET_ID}'

        [return_code, _, _] = tasks.run_command(command)

        assert return_code == 0
