...
from tests.end_to_end.target_snowflake.tap_yugabyte import TapYugabyte


LARGE_VARCHAR_LENGTH = 16_777_217


YUGABYTE_FASTSYNC_COMPARISON_COLUMNS = [
    {
        'name': 'cid',
...
class TestPartialSyncYugabyteToSF(TapYugabyte):
    """
    ...
    def setUp(self):
        self.table = 'edgydata'
        self.column = 'cid'
        super().setUp(tap_id='yugabyte_to_sf', target_id='snowflake')
        ...
        self.tap_parameters = {
            ...
            'table': self.table,
            'column': self.column,
            'comparison_columns': YUGABYTE_FASTSYNC_COMPARISON_COLUMNS,
        }
    ...
    def test_partial_sync_widens_existing_native_varchar(self):
        ...
        target_schema = (
            f'PPW_E2E_TAP_YUGABYTE{self.e2e_env.sf_schema_postfix}'
        ).upper()
    ...
    def test_partial_sync_yugabyte_to_sf(self):
        """
        Test partial sync table from YugabyteDB to Snowflake
        """
        ...
        assertions.assert_partial_sync_rows_in_target(
            self.e2e_env, 'yugabyte', self.table, column_to_check, primary_key, expected_records_for_column
        )
    ...
    def test_unbounded_empty_range_removes_target_only_row(self):
        ...
        source_max = self.e2e_env.run_query_tap_yugabyte(
        ...
        self.e2e_env.run_query_target_snowflake(
            f'UPDATE ppw_e2e_tap_yugabyte{self.e2e_env.sf_schema_postfix}.{self.table} '
    ...
    def test_partial_sync_if_there_is_additional_column_in_source(self):
        """
        Test partial sync table from YugabyteDB to Snowflake if there are additional columns in source
        """
        ...
        primary_key = self.column
        assertions.assert_partial_sync_rows_in_target(
            self.e2e_env, 'yugabyte', self.table, additional_column, primary_key, expected_records_for_column
        )

    def test_partial_sync_if_there_is_additional_column_in_target(self):
        """
        Test partial sync table from YugabyteDB to Snowflake if there are additional columns in target
        """
    ...
    def test_partial_sync_if_record_is_deleted_from_the_source_and_hard_delete(self):
        """
        Test partial sync table from YugabyteDB to Snowflake if hard delete is selected and a record is
        deleted from the source
        """
        self.e2e_env.delete_record_from_source('yugabyte', self.table, 'WHERE cid=5')
