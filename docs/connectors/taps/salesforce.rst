...
enable FastSync FullSync or PartialSync. Salesforce's default
``data_flattening_max_level: 10`` remains valid. Set the level to ``0`` only when
nested values should remain unflattened and use the explicit-v3 ``VARIANT``
mapping. See :ref:`snowflake_iceberg`.
