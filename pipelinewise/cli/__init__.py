...
def _validate_data_diff_arguments(args):
    ...
    if args.command == "rerun_data_diff_check":
        if not args.run_id:
            raise CommandSpecificArgumentsException("You must specify the failed data-diff run using --run-id")
        if not args.remediation_ref:
            raise CommandSpecificArgumentsException(
                "You must specify the repair or incident reference using --remediation-ref"
            )
...
def main():
    ...
    parser.add_argument("--check", type=str, default=None, help="Data-diff check name, logical key, or version ID")
    parser.add_argument(
        "--output-format", choices=["table", "json"], default="table", help="Output format for list commands"
    )
    parser.add_argument(
        "--include-versioned", default=False, action="store_true", help="Include superseded definition revisions"
    )
    parser.add_argument("--run-id", default=None, help="Failed data-diff run ID to reproduce for remediation")
    parser.add_argument(
        "--remediation-ref", default=None, help="Ticket, incident, or change reference for a remediation rerun"
    )
    parser.add_argument(
        "--all",
        default=False,
