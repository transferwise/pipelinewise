...
class FastSyncTargetSnowflake:
    ...
    def partial_hard_delete(self, schema, table, where_clause_sql):
        self.query(f'DELETE FROM {schema}."{table.upper()}"{where_clause_sql} AND _SDC_DELETEd_AT IS NOT NULL')
        ...
        if transformations:
            ...
            for trans_item in transformations:
                ...
                if trans_item["conditions"]:
                    sql = f"UPDATE {full_qual_table_name} SET {trans_item['trans']} WHERE {trans_item['conditions']};"
