...
class FastSyncTapMySql:
    ...
    def __init__(self, connection_config: dict, tap_type_to_target_type: Callable, target_quote=None):
        ...
        self.connection_config["use_gtid"] = connection_config.get("use_gtid", DEFAULT_USE_GTID)
        self.connection_config["engine"] = connection_config.get("engine", MYSQL_ENGINE)
    ...
    def open_connections(self):
        ...
        self.conn: Connection = pymysql.connect(**conn_params, cursorclass=pymysql.cursors.DictCursor, ssl={"": True})
        self.conn_unbuffered: Connection = pymysql.connect(
            **conn_params, cursorclass=pymysql.cursors.SSCursor, ssl={"": True}
        )
    ...
    def fetch_current_incremental_key_pos(self, table, replication_key):
        ...
        """
        result = self.query(f"SELECT MAX({replication_key}) AS key_value FROM {table}")
        if not result:
            raise Exception(f"Cannot get replication key value for table: {table}")
    ...
    def copy_table(  # noqa: PLR0913, PLR0917
        self,
        table_name,
        path,
        max_num=None,
        ...
        FROM `{}`.`{}` {}
        """.format(
            ",".join(column_safe_sql_values), table_dict["schema_name"], table_dict["table_name"], where_clause_sql
        )
    ...
    def export_source_table_data(self, args: Namespace, tap_id: str, where_clause_sql: str = "") -> list:
