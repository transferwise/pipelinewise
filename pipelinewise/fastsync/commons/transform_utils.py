...
class TransformationHelper:
        ...
        for trans_item in transformations:
            if trans_item.get("tap_stream_name").lower() == stream_name.lower():
                ...
                if transform_type == TransformationType.SET_NULL:
                    trans_map.append({"trans": f"{column} = NULL", "conditions": conditions})
                    ...
                    trans_map.append({"trans": f"{column} = 0", "conditions": conditions})

                elif transform_type.value.startswith("MASK-STRING-SKIP-ENDS-"):
                    trans_map.append(
                        {
                            "trans": cls.__mask_string_skip_ends_to_sql(transform_type, column, sql_flavor),
                            "conditions": conditions,
                        }
                    )

                elif transform_type == TransformationType.MASK_HIDDEN:
                    trans_map.append({"trans": f"{column} = 'hidden'", "conditions": conditions})

        return trans_map

    @classmethod
    def __conditions_to_sql(cls, transform_conditions: List[Dict], sql_flavor: SQLFlavor) -> Optional[str]:
        """
            ...
            if "equals" in condition:
                ...
                if sql_flavor == SQLFlavor.SNOWFLAKE:
                    ...
                    raise NotImplementedError(
                        f"regex_match conditional transformation in {sql_flavor.value} SQL flavor not implemented!"
                    )
    ...
    def __hash_to_sql(cls, column: str, sql_flavor: SQLFlavor) -> str:
        ...
        if sql_flavor == SQLFlavor.SNOWFLAKE:
            ...
            raise NotImplementedError(f"HASH transformation in {sql_flavor.value} SQL flavor not implemented!")
    ...
    def __hash_skip_first_to_sql(cls, transform_type: TransformationType, column: str, sql_flavor: SQLFlavor) -> str:
        ...
        if sql_flavor == SQLFlavor.SNOWFLAKE:
            ...
            trans = (
                "{0} = CONCAT(SUBSTRING({0}, 1, {1}), ENCODE(DIGEST(SUBSTRING({0}, {1} + 1), 'sha256'), 'hex'))".format(
                    column, skip_first_n
                )
            )
        else:
            raise NotImplementedError(
                f"HASH-SKIP-FIRST-{skip_first_n} transformation in {sql_flavor.value} SQL flavor not implemented!"
            )
    ...
    def __mask_date_to_sql(cls, column: str, sql_flavor: SQLFlavor) -> str:
        ...
        """
        if sql_flavor == SQLFlavor.SNOWFLAKE:
            trans = f"{column} = TIMESTAMP_NTZ_FROM_PARTS(DATE_FROM_PARTS(YEAR({column}), 1, 1),TO_TIME({column}))"
            ...
            raise NotImplementedError(f"MASK-DATE transformation in {sql_flavor.value} SQL flavor not implemented!")
    ...
    def __mask_string_skip_ends_to_sql(
        ...
        if sql_flavor == SQLFlavor.SNOWFLAKE:
            ...
            raise NotImplementedError(
                f"MASK-STRING-SKIP-ENDS transformation in {sql_flavor.value} SQL flavor not implemented!"
            )
