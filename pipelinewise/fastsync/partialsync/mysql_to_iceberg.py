#!/usr/bin/env python3
"""
Partial sync from MySQL to Apache Iceberg on S3
"""
import os
...
from pipelinewise.fastsync.mysql_to_iceberg import REQUIRED_CONFIG_KEYS, tap_type_to_target_type

LOGGER = Logger().get_logger(__name__)


def partial_sync_table(table: tuple, args: Namespace) -> Union[bool, str]:
    """Partial sync table for MySQL to Iceberg"""
    ...
    try:
        ...
        iceberg_types = mysql.map_column_types_to_target(table_name)
...
def main_impl():
    ...
    LOGGER.info(
        '''
        -------------------------------------------------------
        STARTING PARTIAL SYNC (MySQL to Iceberg)
        -------------------------------------------------------
