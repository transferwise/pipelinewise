...
export class TapMysqlProperties extends React.PureComponent {
      ...
      if (stream) {
        ...
        return (
          <Grid>
            <Table
              items={columns}
              headerComponent={TapMysqlProperties.columnsTableHeader}
              bodyComponent={TapMysqlProperties.columnsTableBody}
  ...
  render() {
    ...
    try {
      ...
      if (streams) {
        const stream = streams.find(s => s['tap_stream_id'] === activeStreamId);
        columns = stream ? TapMysqlProperties.getColumnsFromStream(stream, {}) : []
      ...
      return <Alert bsStyle="danger" className="full-swidth"><strong>Error!</strong> Doesn't look like valid MySQL tap properties: {e.toString()}</Alert>
    }
  }
}

TapMysqlProperties.propTypes = {
...
const withReducer = injectReducer({ key: 'tapMysql', reducer });
const withSaga = injectSaga({ key: 'tapMysql', saga });
