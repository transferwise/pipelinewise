...
export class TapKafkaProperties extends React.PureComponent {
  static streamsTableHeader(props) {
    ...
    return (
      <tr>
        ...
        <th><FormattedMessage {...messages.topic} /></th>
        <th></th>
      </tr>
    )
  }
  ...
  static streamsTableBody(props) {
    ...
    if (isNew) {
      streamChangeDescription = <FormattedMessage {...messages.newTopic} />
    } else if (isModified) {
    ...
    return (
      <tr className={`${isSelected ? "table-active" : ""} ${streamChangeDescription ? "table-warning" : ""}`} onClick={() => props.onItemSelect(stream, streamId)}>
        ...
        <td>{item['tap_stream_id']}</td>
        <td>{item['is-new'] ? <FormattedMessage {...messages.newTopic} /> : ''}</td>
      </tr>
    )
  }
  ...
  renderStreamColumns(streams, activeStream, activeStreamId, delegatedProps) {
    if (activeStreamId) {
      ...
      if (stream) {
        ...
        const columns = TapKafkaProperties.getColumnsFromStream(stream, { activeStream, replicationKey, tableKeys })
        return (
          <Grid>
            <Table
              items={columns}
              headerComponent={TapKafkaProperties.columnsTableHeader}
              bodyComponent={TapKafkaProperties.columnsTableBody}
  ...
  render() {
    ...
    try {
      ...
      if (streams) {
        const stream = streams.find(s => s['tap_stream_id'] === activeStreamId);
        columns = stream ? TapKafkaProperties.getColumnsFromStream(stream, {}) : []
        ...
        <Grid>
          ...
          {Array.isArray(streams) ?
            <Grid>
              <Row>
                <Col md={12}> 
                  <strong><FormattedMessage {...messages.topicsToReplicate} /></strong>&nbsp;
                  {streams ? `(${selectedStreams.length} / ${streams.length} selected)` : ''}
                  <Grid className="table-wrapper-scroll-y">
                    <Table
                      items={streams}
                      selectedItem={activeStreamId}
                      headerComponent={TapKafkaProperties.streamsTableHeader}
                      bodyComponent={TapKafkaProperties.streamsTableBody}
      ...
      return <Alert bsStyle="danger" className="full-swidth"><strong>Error!</strong> Doesn't look like valid Kafka tap properties: {e.toString()}</Alert>
    }
  }
}

TapKafkaProperties.propTypes = {
...
const withReducer = injectReducer({ key: 'tapKafka', reducer });
const withSaga = injectSaga({ key: 'tapKafka', saga });
