...
- Require PipelineWise-created managed Iceberg v3 targets from earlier releases
  to set
  `ICEBERG_MERGE_ON_READ_BEHAVIOR = 'DISABLED'` at table level before writes
- Require every string column in those existing PipelineWise-created managed
  Iceberg v3 targets to use
...
- Preserve 64-bit approximate numerics as Iceberg `DOUBLE` on explicit v3 routes
  without changing native or fixed-point mappings
...
- Reject conversion when dependent objects or metadata cannot be preserved
...
- Preserve existing `TIMESTAMP_TZ` data when managed Iceberg v3 Singer schema
  evolution requests precision-qualified `TIMESTAMP_NTZ(6)`
...
- Restrict target-snowflake timestamp preservation to compatible existing
  timestamp columns when Singer requests `TIMESTAMP_NTZ(6)`
...
- Cover committed missing-target CTAS response loss with the production query-history polling budget
...
The 0.80.0 release and tag were deleted. Use 0.81.0, which includes every
still-supported change from 0.80.0.
