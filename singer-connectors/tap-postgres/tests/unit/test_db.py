...
class TestDbFunctions(unittest.TestCase):
    ...
    def test_value_to_singer_value(self):
        ...
        self.assertEqual(
            db.selected_value_to_singer_value_impl("2020-05-01T12:00:00-0800", "timestamp with time zone"),
        ...
        self.assertEqual(
            db.selected_value_to_singer_value_impl(datetime.date(2022, 11, 11), "foo"), "2022-11-11T00:00:00+00:00"
        )
        self.assertEqual(db.selected_value_to_singer_value_impl(datetime.time(11, 11, 11), "foo"), "11:11:11")
    ...
    def test_prepare_columns_for_select_sql_with_timestamp_ntz_column(self):
        self.assertEqual(
            ...
            db.prepare_columns_for_select_sql(
                "my_column", {("properties", "my_column"): {"sql-datatype": "timestamp without time zone"}}
            ),
        )
    ...
    def test_prepare_columns_for_select_sql_with_timestamp_tz_column(self):
        self.assertEqual(
            ...
            'ELSE  "my_column"  END AS  "my_column" ',
            db.prepare_columns_for_select_sql(
                "my_column", {("properties", "my_column"): {"sql-datatype": "timestamp with time zone"}}
            ),
        )
    ...
    def test_prepare_columns_for_select_sql_with_not_timestamp_column(self):
        self.assertEqual(
            ' "my_column" ',
            db.prepare_columns_for_select_sql("my_column", {("properties", "my_column"): {"sql-datatype": "int"}}),
        )

    def test_prepare_columns_for_select_sql_with_column_not_in_map(self):
        self.assertEqual(
            ' "my_column" ',
            db.prepare_columns_for_select_sql("my_column", {("properties", "nope"): {"sql-datatype": "int"}}),
        )
