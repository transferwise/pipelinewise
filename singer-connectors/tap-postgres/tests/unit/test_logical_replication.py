...
class TestLogicalReplication(unittest.TestCase):
    ...
    def test_streams_to_wal2json_tables(self):
        ...
        streams = [
            {"metadata": [{"metadata": {"schema-name": "public"}}], "table_name": "dummy_table"},
            {"metadata": [{"metadata": {"schema-name": "public"}}], "table_name": "CaseSensitiveTable"},
            {"metadata": [{"metadata": {"schema-name": "public"}}], "table_name": "Case Sensitive Table With Space"},
            {"metadata": [{"metadata": {"schema-name": "CaseSensitiveSchema"}}], "table_name": "dummy_table"},
            {
            ...
            {"metadata": [{"metadata": {"schema-name": "public"}}], "table_name": "table_with_comma_,"},
            {"metadata": [{"metadata": {"schema-name": "public"}}], "table_name": "table_with_quote_'"},
        ]
    ...
    def test_generate_replication_slot_name(self):
        ...
        self.assertEqual(logical_replication.generate_replication_slot_name("some_db"), "pipelinewise_some_db")
    ...
    def test_locate_replication_slot_by_cur(self):
        ...
        self.assertEqual(
            logical_replication.locate_replication_slot_by_cur(cursor, "some_db", "some_tap"), "pipelinewise_some_db"
        )
        ...
        self.assertEqual(
            logical_replication.locate_replication_slot_by_cur(cursor, "some_db", "some_tap"),
            "pipelinewise_some_db_some_tap",
        )

        # Should return v15 style replication slot if tap_id not provided and the v15 slot exists
        cursor = PostgresCurReplicationSlotMock(existing_slot_name="pipelinewise_some_db")
        self.assertEqual(logical_replication.locate_replication_slot_by_cur(cursor, "some_db"), "pipelinewise_some_db")
    ...
    def test_consume_with_message_payload_is_not_json_expect_same_state(self):
        output = logical_replication.consume_message(
            [], {}, self.WalMessage(payload="this is an invalid json message", data_start=None), None, {}
        )
    ...
    def test_consume_with_message_stream_in_payload_is_not_selected_expect_same_state(self):
        output = logical_replication.consume_message(
            ...
            {},
            self.WalMessage(payload='{"schema": "myschema", "table": "notmytable"}', data_start="some lsn"),
            None,
            {},
        )
    ...
    def test_consume_with_truncate_action_returns_state_unchanged(self):
        ...
        output = logical_replication.consume_message(
            [{"tap_stream_id": "myschema-mytable"}],
            {},
            self.WalMessage(payload='{"action":"T", "schema": "myschema", "table": "mytable"}', data_start="some lsn"),
            None,
            {},
        )
    ...
    def test_consume_with_begin_action_without_schema_table_returns_state_unchanged(self):
        ...
        output = logical_replication.consume_message(
            ...
            {},
            self.WalMessage(
                payload='{"action":"B","xid":12345,"timestamp":"2026-04-06 12:00:00.000000+00"}', data_start="some lsn"
            ),
            None,
            {},
        )
    ...
    def test_consume_with_commit_action_without_schema_table_returns_state_unchanged(self):
        ...
        output = logical_replication.consume_message(
            ...
            {},
            self.WalMessage(
                payload='{"action":"C","xid":12345,"timestamp":"2026-04-06 12:00:00.000000+00"}', data_start="some lsn"
            ),
            None,
            {},
        )
        ...
        streams = [
            {
                ...
                "metadata": [
                    {
                        "breadcrumb": [],
                        "metadata": {"is-view": False, "table-key-properties": ["id"], "schema-name": "myschema"},
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "metadata": {"sql-datatype": "datetime", "inclusion": "available", "selected": True},
                    },
                ],
            }
        ]
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_ntz_value_as_datetime_max(self):
        output = logical_replication.selected_value_to_singer_value_impl(
            datetime(9999, 12, 31, 23, 59, 59, 999999), "timestamp without time zone", None
        )
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_tz_value_as_datetime_expect_iso_format(self):
        output = logical_replication.selected_value_to_singer_value_impl(
            datetime(2020, 9, 1, 23, 10, 59, tzinfo=tzoffset(None, -3600)), "timestamp with time zone", None
        )
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_tz_value_as_datetime_min(self):
        output = logical_replication.selected_value_to_singer_value_impl(
            datetime(1, 1, 1, 0, 0, 0, 123, tzinfo=tzoffset(None, 14400)), "timestamp with time zone", None
        )
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_tz_value_as_datetime_max(self):
        output = logical_replication.selected_value_to_singer_value_impl(
            datetime(9999, 12, 31, 23, 59, 59, 999999, tzinfo=tzoffset(None, -14400)), "timestamp with time zone", None
        )
    ...
    def test_row_to_singer_message_without_sql_datatype(self):
        stream = {
            "stream": "my_stream",
        }
        ...
        output = logical_replication.row_to_singer_message(
            stream, row, 1000, columns, datetime(2020, 9, 1, 10, 10, 10, tzinfo=tzoffset(None, 0)), md_map, None
        )
    ...
    def test_selected_value_to_singer_value_impl_with_null_json_returns_None(self):
        output = logical_replication.selected_value_to_singer_value_impl(None, "json", None)

        self.assertEqual(None, output)

    def test_selected_value_to_singer_value_impl_with_empty_json_returns_empty_dict(self):
        output = logical_replication.selected_value_to_singer_value_impl("{}", "json", None)
    ...
    def test_selected_value_to_singer_value_impl_with_null_jsonb_returns_None(self):
        output = logical_replication.selected_value_to_singer_value_impl(None, "jsonb", None)

        self.assertEqual(None, output)

    def test_selected_value_to_singer_value_impl_with_empty_jsonb_returns_empty_dict(self):
        output = logical_replication.selected_value_to_singer_value_impl("{}", "jsonb", None)
    ...
    def test_add_automatic_properties_if_debug_lsn_is_off(self):
        ...
        expected_stream = {
            "schema": {"properties": {"_sdc_deleted_at": {"format": "date-time", "type": ["null", "string"]}}}
        }
    ...
    def test_impl_with_sql_datatype_is_date_with_invalid_elem_raises_exception(self):
        ...
        conn_info = None
        self.assertRaises(
            ValueError, logical_replication.selected_value_to_singer_value_impl, elem, og_sql_datatype, conn_info
        )
    ...
    def test_consume_message_if_payload_kind_insert_or_update(self, *args):
        ...
        expected_output = {
            "bookmarks": {
                "foo-bar": {"foo": "bar", "lsn": data_start, "version": state["bookmarks"]["foo-bar"]["version"]}
            }
        }
    ...
    def test_sync_tables_raises_exception_if_psycopg2_programming_error(
        ...
        expected_message = f"Unable to start replication with logical replication (slot {test_replication_slot})"
    ...
    @patch("tap_postgres.sync_strategies.logical_replication.get_pg_version")
    @patch("psycopg2.connect")
    def test_sync_tables_raise_exception_if_error_in_message_read(self, mocked_connect, mocked_version, _):
    ...
    def test_sync_tables_if_break_at_end_lsn_and_msg_data_start_greater_than_end_lsn(
        ...
        expected_log_message = (
            "INFO:tap_postgres:Breaking - latest wal message "
    ...
    def test_sync_tables_if_msg_and_some_specific_cases_for_lsn(
        ...
        mocked_send_feedback.assert_called_with(
            write_lsn=test_message.data_start, flush_lsn=test_message.data_start, reply=True, force=True
        )
