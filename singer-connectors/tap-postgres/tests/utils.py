...
def get_test_connection_config(target_db="postgres", use_secondary=False):
    try:
        ...
        raise Exception("set TAP_POSTGRES_HOST, TAP_POSTGRES_USER, TAP_POSTGRES_PASSWORD, TAP_POSTGRES_PORT") from exc
    ...
    if use_secondary:
        try:
            ...
            raise Exception("set TAP_POSTGRES_SECONDARY_HOST, TAP_POSTGRES_SECONDARY_PORT") from exc
...
def get_test_connection(target_db="postgres", superuser=False):
    ...
    user, password = (
        ("postgres", conn_config["postgres_password"]) if superuser else (conn_config["user"], conn_config["password"])
    )

    conn_string = "host='{}' dbname='{}' user='{}' password='{}' port='{}'".format(
        conn_config["host"], conn_config["dbname"], user, password, conn_config["port"]
    )
