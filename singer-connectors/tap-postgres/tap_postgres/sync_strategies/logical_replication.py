...
def create_array_elem(elem, sql_datatype, conn_info):  # noqa: C901, PLR0912 - PostgreSQL array datatype map
    if elem is None:
        return None
...
def selected_value_to_singer_value_impl(  # noqa: C901, PLR0911, PLR0912, PLR0915 - datatype conversion map
    ...
    if sql_datatype == "date":
        ...
        try:
            ...
            if match and int(match.group(1)) > 9999:
                LOGGER.warning("datetimes cannot handle years past 9999, returning %s for %s", FALLBACK_DATE, elem)
...
def selected_value_to_singer_value(elem, sql_datatype, conn_info):
    ...
    if sql_datatype.find("[]") > 0:
        ...
        return list(
            map(lambda elem: selected_array_to_singer_value(elem, sql_datatype, conn_info), (cleaned_elem or []))
        )
...
def consume_message(streams, state, msg, time_extracted, conn_info):
    ...
    if action not in {"I", "U", "D"}:
        LOGGER.debug(
            "Skipping non-row wal2json message: action=%s, lsn=%s",
            action,
    ...
    desired_columns = {
        c for c in target_stream["schema"]["properties"].keys() if sync_common.should_sync_column(stream_md_map, c)
    }
    ...
    record_message = row_to_singer_message(
        target_stream, col_vals, stream_version, col_names, time_extracted, stream_md_map, conn_info
    )
...
def sync_tables(  # noqa: C901, PLR0912, PLR0915 - logical replication event loop
    conn_info, logical_streams, state, end_lsn, state_file
):
    ...
    try:
        LOGGER.info("Request wal streaming from %s to %s (slot %s)", int_to_lsn(start_lsn), int_to_lsn(end_lsn), slot)
        ...
        while True:
            ...
            if msg:
                ...
                if lsn_currently_processing is None:
                    ...
                    LOGGER.info(
                        "Confirming write up to %s, flush to %s", int_to_lsn(lsn_to_flush), int_to_lsn(lsn_to_flush)
                    )
