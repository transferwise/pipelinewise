...
def sync_method_for_streams(streams, state, default_replication_method):
    ...
    for stream in streams:
        ...
        desired_columns = [
            c for c in stream["schema"]["properties"].keys() if sync_common.should_sync_column(md_map, c)
        ]
...
def sync_logical_streams(conn_config, logical_streams, state, end_lsn, state_file):
    ...
    if logical_streams:
        LOGGER.info(
            "Pure Logical Replication upto lsn %s for (%s)", end_lsn, [s["tap_stream_id"] for s in logical_streams]
        )

        logical_streams = [
            logical_replication.add_automatic_properties(s, conn_config.get("debug_lsn", False))
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if citext_array_oid:
                psycopg2.extensions.register_type(
                    psycopg2.extensions.new_array_type((citext_array_oid[0],), "CITEXT[]", psycopg2.STRING)
                )
            ...
            # json and jsonb
            psycopg2.extras.register_default_json(loads=str)
            psycopg2.extras.register_default_jsonb(loads=str)
            ...
            for oid in cur.fetchall():
                enum_oid = oid[0]
                psycopg2.extensions.register_type(
                    psycopg2.extensions.new_array_type((enum_oid,), f"ENUM_{enum_oid}[]", psycopg2.STRING)
                )
