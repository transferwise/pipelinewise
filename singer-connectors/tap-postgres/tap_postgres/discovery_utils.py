...
SELECT
  ...
  information_schema._pg_char_max_length(
    ...
    END, information_schema._pg_truetypmod(a.*, pgt.*)
...
def discover_columns(connection, table_info):
    ...
    for schema_name in table_info.keys():
        for table_name in table_info[schema_name].keys():
            ...
            for c_name, column in columns.items():
                mdata = write_sql_data_type_md(mdata, column)
...
def schema_for_column_datatype(  # noqa: C901, PLR0911, PLR0912, PLR0915 - PostgreSQL datatype map
    col,
):
    """
...
def schema_for_column(col_info):  # noqa: C901, PLR0912, PLR0915 - schema construction follows datatype branches
    """
...
def write_sql_data_type_md(mdata, col_info):
    ...
    if col_info.sql_data_type == "bit" and col_info.character_maximum_length > 1:
        mdata = metadata.write(
            mdata, ("properties", c_name), "sql-datatype", f"bit({col_info.character_maximum_length})"
        )
    else:
