...
def fetch_current_gtid_pos(mysql_conn: MySQLConnection, engine: str) -> str:  # noqa: PLR0912
    """
...
def row_to_singer_record(  # noqa: C901, PLR0912 - conversion follows source datatype branches
    catalog_entry, version, db_column_map, row, time_extracted
):
    row_to_persist = {}
...
def calculate_gtid_bookmark(  # noqa: PLR0912 - engine-specific GTID validation
...
def __get_diff_in_columns_list(
    ...
    binlog_columns_filtered = [
        col.name
        for col in binlog_event.columns
        if col.name and not (re.match(r"__dropped_col_\d+__", str(col.name)) or col.name in ignore_columns)
    ]

    return set(binlog_columns_filtered).difference(schema_properties)


def _run_binlog_sync(  # noqa: C901, PLR0912, PLR0915 - binlog event loop coordinates state and messages
