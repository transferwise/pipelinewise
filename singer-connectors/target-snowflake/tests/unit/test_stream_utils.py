...
class TestSchemaUtils(unittest.TestCase):
    ...
    def test_get_schema_names_from_config(self):
        ...
        self.assertEqual(
            stream_utils.get_schema_names_from_config({"default_target_schema": "test_schema_for_default"}),
            ["test_schema_for_default"],
        )

        # Schema mapping should support multiple schemas
        self.assertEqual(
            stream_utils.get_schema_names_from_config(
                {
                    "schema_mapping": {
                        "stream_1": {"target_schema": "test_schema_for_stream_1"},
                        "stream_2": {"target_schema": "test_schema_for_stream_2"},
                    }
                }
            ),
            ["test_schema_for_stream_1", "test_schema_for_stream_2"],
        )
        ...
        self.assertEqual(
            ...
            ["test_schema_for_default", "test_schema_for_stream_1", "test_schema_for_stream_2"],
        )
    ...
    def test_adjust_timestamps_in_record(self):
        ...
        schema = {
            "properties": {
                ...
                "key2": {"anyOf": [{"type": ["null", "string"], "format": "date"}, {"type": ["null", "string"]}]},
                "key3": {
                    "type": ["null", "string"],
                    "format": "date-time",
                },
                "key4": {"anyOf": [{"type": ["null", "string"], "format": "time"}, {"type": ["null", "string"]}]},
                "key5": {
                    "type": ["null", "string"],
                },
                "key6": {
    ...
    def test_float_to_decimal(self):
        ...
        self.assertEqual(
            stream_utils.float_to_decimal([1.123, 2.234, 3.345, "this is not float"]),
            [Decimal("1.123"), Decimal("2.234"), Decimal("3.345"), "this is not float"],
        )

        # Nested dictionary
        self.assertEqual(
            stream_utils.float_to_decimal(
                {
                    "k1": 1.123,
                    "k2": 2.234,
                    "k3": [1.123, 2.234, 3.345, "this is not float"],
                    "k4": {"j1": "foo", "j2": [1.123, 2.234, 3.345, "this is not float again"], "j3": {"k1": 1.123}},
                }
            ),
            {
                "k1": Decimal("1.123"),
    ...
    def test_get_incremental_key(self):
        ...
        with self.assertRaises(UnexpectedMessageTypeException):
            stream_utils.get_incremental_key({"type": "RECORD", "stream": "some-stream", "record": {}})
