...
class TestDBSync(unittest.TestCase):
    ...
    def test_config_validation(self):
        ...
        config_with_schema_mapping["schema_mapping"] = {"dummy_stream": {"target_schema": "dummy_schema"}}
    ...
    def test_create_query_tag(self):
        ...
        self.assertEqual(
            db_sync.create_query_tag(
                "Loading into {{database}}.{{schema}}.{{table}}", database=None, schema=None, table=None
            ),
            "Loading into ..",
        )
        ...
        self.assertEqual(
            json.loads(json_query_tag), {"database": "test_database", "schema": "test_schema", "table": "test_table"}
        )
        ...
        # Load the generated JSON formatted query tag to make sure it's a valid JSON
        self.assertEqual(
            json.loads(json_query_tag), {"database": 'test"database', "schema": 'test"schema', "table": 'test"table'}
        )
    ...
    def test_parallelism(self, query_patch):
        ...
        self.assertEqual(
            db_sync.DbSync({**minimal_config, **external_stage_with_parallel}).connection_config["parallelism"], 5
        )

        # Using table stages should allow parallelism
        table_stage_with_parallel = {"parallelism": 5}
        self.assertEqual(
            db_sync.DbSync({**minimal_config, **table_stage_with_parallel}).connection_config["parallelism"], 5
        )
    ...
    def test_copy_to_archive(self, query_patch, copy_object_patch):
        ...
        s3_config = {"archive_load_files_s3_bucket": "custom-bucket", "archive_load_files_s3_prefix": "custom-prefix"}
        dbsync = db_sync.DbSync({**minimal_config, **s3_config})
        dbsync.copy_to_archive("source/file", "tap/schema/file", {"meta": "data"})
    ...
    def test_merge_failure_message(self, load_file_merge_patch, query_patch):
        ...
        stream_schema_message = {
            "stream": "dummy_stream",
            "schema": {"properties": {"id": {"type": ["integer"]}, "c_str": {"type": ["null", "string"]}}},
            "key_properties": ["id"],
        }
    ...
    def test_sync_table_with_no_changes_to_pk(self, query_patch):
        ...
        table_cache = [
            ...
            {"SCHEMA_NAME": "DUMMY-SCHEMA", "TABLE_NAME": "TABLE1", "COLUMN_NAME": "C_STR", "DATA_TYPE": "TEXT"},
        ]
    ...
    def test_sync_table_with_new_pk_in_stream(self, query_patch):
        ...
        table_cache = [
            {"SCHEMA_NAME": "DUMMY-SCHEMA", "TABLE_NAME": "TABLE1", "COLUMN_NAME": "ID", "DATA_TYPE": "NUMBER"},
            {"SCHEMA_NAME": "DUMMY-SCHEMA", "TABLE_NAME": "TABLE1", "COLUMN_NAME": "C_STR", "DATA_TYPE": "TEXT"},
            {"SCHEMA_NAME": "DUMMY-SCHEMA", "TABLE_NAME": "TABLE1", "COLUMN_NAME": "NAME", "DATA_TYPE": "TEXT"},
        ]
        query_patch.side_effect = [
            [{"type": "CSV"}],  # SHOW FILE FORMATS
            [],  # SHOW TERSE ICEBERG TABLES (not Iceberg)
    ...
    def test_column_type_mapping_iceberg(self):
        ...
        for key, expected in unchanged.items():
            self.assertEqual(
                mapper(self.json_types[key], is_iceberg_table=True),
                expected,
    ...
    def test_column_clause_iceberg(self):
        ...
        # variant → text
        self.assertEqual(
            db_sync.column_clause("my_obj", self.json_types["obj"], is_iceberg_table=True), '"MY_OBJ" text'
        )
        # integer → number(19,0)
        self.assertEqual(
            db_sync.column_clause("my_int", self.json_types["int"], is_iceberg_table=True), '"MY_INT" number(19,0)'
        )
        # Standard (non-iceberg) path is unchanged
        self.assertEqual(db_sync.column_clause("my_obj", self.json_types["obj"]), '"MY_OBJ" variant')
        self.assertEqual(db_sync.column_clause("my_int", self.json_types["int"]), '"MY_INT" number')
    ...
    def test_update_columns_iceberg_adds_new_column(self, query_patch):
        ...
        stream_schema_message = {
            "stream": "public-table1",
            "schema": {"properties": {"id": {"type": ["integer"]}, "new_col": {"type": ["null", "string"]}}},
            "key_properties": ["id"],
        }
    ...
    def test_update_columns_iceberg_number_no_spurious_alter(self, query_patch):
        ...
        alter_calls = [
            str(c) for c in query_patch.call_args_list if "ADD COLUMN" in str(c) or "RENAME COLUMN" in str(c)
        ]
        self.assertEqual(len(alter_calls), 0, msg="NUMBER should not be re-altered for number(19,0) on Iceberg table")
    ...
    def test_update_columns_iceberg_text_not_altered_for_variant_schema(self, query_patch):
        ...
        self.assertEqual(
            len(alter_calls), 0, msg="TEXT should not be re-altered when schema is variant on an Iceberg table"
        )
    ...
    @patch("target_snowflake.db_sync.DbSync.get_tables")
    @patch("target_snowflake.db_sync.DbSync.query")
    def test_sync_table_creates_regular_table_when_iceberg_create_false(
        self, query_patch, get_tables_patch, grant_patch
    ):
