...
class TestIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.s3_client = aws_session.client(
            "s3", region_name=self.config.get("s3_region_name"), endpoint_url=self.config.get("s3_endpoint_url")
        )
            ...
            expected_table_two = [
                {
                    ...
                    "C_ISO_DATE": datetime.date(2019, 2, 1),
                },
                {
                    "C_INT": 2,
                    "C_PK": 2,
                    ...
                    "C_ISO_DATE": datetime.date(2019, 2, 10),
                },
            ]
        else:
            expected_table_two = [
    ...
    def test_run_query(self):
        ...
        self.assertEqual(snowflake.query("SELECT 1 col1, 2 col2"), [{"COL1": 1, "COL2": 2}])

        # Running multiple SQLs should return the result of the last query
        self.assertEqual(
            snowflake.query(["SELECT 1 col1, 2 col2", "SELECT 3 col1, 4 col2", "SELECT 5 col1, 6 col2"]),
            [{"COL1": 5, "COL2": 6}],
        )

        # Running multiple SQLs should return empty list if the last query returns zero record
        self.assertEqual(
            snowflake.query(["SELECT 1 col1, 2 col2", "SELECT 3 col1, 4 col2", "SELECT 5 col1, 6 col2 WHERE 1 = 2"]), []
        )

        # Running multiple SQLs should return the result of the last query even if a previous query returns zero record
        self.assertEqual(
            snowflake.query(["SELECT 1 col1, 2 col2 WHERE 1 =2 ", "SELECT 3 col1, 4 col2", "SELECT 5 col1, 6 col2"]),
            [{"COL1": 5, "COL2": 6}],
        )
        ...
        self.assertEqual(
            snowflake.query(
                [
                    "SELECT 1 col1, 2 col2 WHERE 1 = 2 ",
    ...
    def test_loading_tables_with_hard_delete(self):
        ...
        # Check if data loaded correctly and metadata columns exist
        self.assert_three_streams_are_into_snowflake(should_metadata_columns_exist=True, should_hard_deleted_rows=True)
    ...
    def test_loading_tables_with_binary_columns_and_hard_delete(self):
        ...
        # Check if data loaded correctly and metadata columns exist
        self.assert_binary_data_are_in_snowflake(table_name="test_binary", should_metadata_columns_exist=True)
    ...
    def test_loading_table_with_reserved_word_as_name_and_hard_delete(self):
        ...
        # Check if data loaded correctly and metadata columns exist
        self.assert_binary_data_are_in_snowflake(table_name='"ORDER"', should_metadata_columns_exist=True)
            ...
            [
                {
                    ...
                    "C_NESTED_OBJECT": (
                        '{"nested_prop_1":"nested_value_1","nested_prop_2":"nested_value_2",'
    ...
    def test_column_name_change(self):
        ...
        previous_column_name = snowflake.query(
            ...
            """.format(self.config.get("dbname", "").upper(), target_schema.upper())
        )[0]["COLUMN_NAME"]
        ...
        self.assertEqual(
            ...
            [
                ...
                },
                {
                    ...
                    "C_ISO_DATE": datetime.date(2019, 2, 15),
                    "C_NEW_COLUMN": "data 2",
                },
            ],
        )
    ...
    def test_quoted_identifiers_ignore_case_session_parameter(self):
        ...
        self.assertEqual(result, [{"Foo": 1, "foo": 1, "FOO": 1}])
    ...
    def test_query_tagging(self):
        ...
        self.config["query_tag"] = (
            f"PPW test tap run at {current_time}. Loading into {{{{database}}}}.{{{{schema}}}}.{{{{table}}}}"
        )
        self.persist_lines_with_cache(tap_lines)
        ...
        self.assertEqual(
            result,
            [
                {"QUERY_TAG": f"PPW test tap run at {current_time}. Loading into {target_db}..", "QUERIES": 4},
                {
                    "QUERY_TAG": (
        ...
        self.assertEqual(result, [{"SHOW_FILE_FORMAT_QUERIES": 1}])
...
""",
    ...
    def test_stream_with_changing_pks_should_succeed(self):
        ...
        rows_count = self.snowflake.query(
            f"select count(1) as _count from {self.config['default_target_schema']}.test_simple_table;"
        )
