...
def get_snowflake_statics(config):
    ...
    if not ("disable_table_cache" in config and config["disable_table_cache"]):
        ...
        db = DbSync(config)
        table_cache = db.get_table_columns(table_schemas=stream_utils.get_schema_names_from_config(config))
...
def persist_lines(  # noqa: C901, PLR0912, PLR0915 - Singer message loop coordinates all stream state
    ...
    for line in lines:
        ...
        if t == "RECORD":
            ...
            if row_count[stream] >= batch_size_rows:
                flush = True
                LOGGER.info("Flush triggered by batch_size_rows (%s) reached in %s", batch_size_rows, stream)
            elif batch_wait_limit_seconds and datetime.utcnow() >= (
                flush_timestamp + timedelta(seconds=batch_wait_limit_seconds)
            ):
                flush = True
                LOGGER.info("Flush triggered by batch_wait_limit_seconds (%s)", batch_wait_limit_seconds)
            ...
            if stream not in schemas or schemas[stream] != new_schema:
                ...
                if config.get("add_metadata_columns") or config.get("hard_delete"):
                    stream_to_sync[stream] = DbSync(
                        config, add_metadata_columns_to_schema(o), table_cache, file_format_type
                    )
                else:
                ...
                if archive_load_files:
                    ...
                    if incremental_key_column_name:
                        ...
                        archive_load_files_data[stream].update(column=incremental_key_column_name, min=None, max=None)
                    else:
                        LOGGER.warning(
    ...
    if sum(row_count.values()) > 0:
        ...
        flushed_state = flush_streams(
            records_to_load, row_count, stream_to_sync, config, state, flushed_state, archive_load_files_data
        )
...
def flush_streams(  # noqa: PLR0913, PLR0917 - existing public connector API
    streams, row_count, stream_to_sync, config, state, flushed_state, archive_load_files_data, filter_streams=None
):
    """
...
def load_stream_batch(  # noqa: PLR0913, PLR0917 - existing parallel worker API
    stream, records, row_count, db_sync, no_compression=False, delete_rows=False, temp_dir=None, archive_load_files=None
):
...
def copy_native_to_iceberg():
    ...
    arg_parser.add_argument(
        "-t", "--fqtn", help="Snowflake fully qualified table name (fqtn) in format database.schema.table"
    )
    arg_parser.add_argument(
        "-e",
        "--eventual",
        help="EVENTUAL type of fqtn : NATIVE (Default) or ICEBERG. The other table type will still exist as a copy",
        default="NATIVE",
    )
