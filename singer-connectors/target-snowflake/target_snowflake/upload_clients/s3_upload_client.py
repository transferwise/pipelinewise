...
class S3UploadClient(BaseUploadClient):
    ...
    def _create_s3_client(self, config=None):
        ...
        return aws_session.client(
            "s3", region_name=config.get("s3_region_name"), endpoint_url=config.get("s3_endpoint_url")
        )
    ...
    def copy_object(self, copy_source: str, target_bucket: str, target_key: str, target_metadata: dict) -> None:
        ...
        self.s3_client.copy_object(
            CopySource=copy_source, Bucket=target_bucket, Key=target_key, Metadata=metadata, MetadataDirective="REPLACE"
        )
