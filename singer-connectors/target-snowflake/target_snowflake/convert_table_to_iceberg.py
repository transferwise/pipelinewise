...
class ConvertTableToIceberg:
    ...
    def __init__(self, connection_config, fqtn=None):
        ...
        self.logger.info("Initializing ConvertTableToIceberg for table: %s", fqtn)
        ...
        query_rename_native=f"ALTER TABLE {fqtn} RENAME TO {fqtn}_NATIVE"
        query_create_iceberg=self.get_create_iceberg(iceberg_columns, pk)
        query_copy_to_iceberg=self.get_query_copy_to_iceberg(native_columns)

        # Rename existing table to _NATIVE
        query=query_rename_native
        self.logger.info(query)
        result = self.query(query)
        self.logger.info(result)

        # Create Iceberg table
        query=query_create_iceberg
        self.logger.info(query)
        result = self.query(query)
        self.logger.info(result)

        # Copy data to Iceberg
        query=query_copy_to_iceberg
        self.logger.info(query)
        result = self.query(query)
        self.logger.info(result)
    ...
    def get_create_iceberg(self, columns, pk):
        ...
        statement = f"CREATE ICEBERG TABLE IF NOT EXISTS {database}.{schema_name}.{table_name} ( "
        ...
        statement += f" CATALOG='{self.connection_config['iceberg_catalog']}'"
        statement += f" CATALOG_TABLE_NAME='{table_name}'"
        statement += f" CATALOG_NAMESPACE='{database}.{schema_name}'"
        statement += f" BASE_LOCATION='{self.connection_config['iceberg_base_location']}'"
        statement += f" DATA_RETENTION_TIME_IN_DAYS=1"
        statement += f" TARGET_FILE_SIZE='16MB'"
        if self.connection_config["iceberg_catalog"].lower() == 'snowflake':
