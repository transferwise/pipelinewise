...
def validate_config(config):
    ...
    snowflake_required_config_keys = ["account", "dbname", "user", "private_key", "warehouse", "file_format"]
...
class DbSync:
    ...
    def __init__(self, connection_config, stream_schema_message=None, table_cache=None, file_format_type=None):
        ...
        if self.stream_schema_message is not None:
            ...
            #
            #   1: 'default_target_schema_select_permissions' key  : USAGE and SELECT privileges will be granted
            #                                                       on every table to a given role for every incoming
            #                                                       stream if not specified in `schema_mapping`.
            #   2: 'target_schema_select_permissions' key          : Roles to grant USAGE and SELECT privileges
            #                                                       defined explicitly for a given stream.
            #                                                       Example config.json:
            #                                                           "schema_mapping": {
            #                                                               "my_tap_stream_id": {
            #                                                                   "target_schema": "my_snowflake_schema",
    ...
    def load_file(self, s3_key, count, size_bytes):
        ...
        # Insert only with COPY command if no primary key
            ...
            try:
                inserts, updates = (
                    self._load_file_copy(s3_key=s3_key, stream=stream, columns_with_trans=columns_with_trans),
                    0,
                )
            except Exception as ex:
                self.logger.error(
    ...
    def create_table_query(self, is_temporary=False):
        ...
        stream_schema_message = self.stream_schema_message
        columns = [
            column_clause(name, schema, is_iceberg_table=False) for (name, schema) in self.flatten_schema.items()
        ]
    ...
    def create_iceberg_table_query(self):
        ...
        stream_schema_message = self.stream_schema_message
        columns = [column_clause(name, schema, is_iceberg_table=True) for (name, schema) in self.flatten_schema.items()]
        ...
        if table_schemas:
            for schema in table_schemas:
                ...
                try:
                    ...
                    if not columns:
                        self.logger.warning(
                            'No columns discovered in the schema "%s"', f"{self.connection_config['dbname']}.{schema}"
                        )
                    else:
    ...
    def update_columns(self, is_iceberg_table=False):
        ...
        columns = list(
            filter(
                lambda x: x["SCHEMA_NAME"] == self.schema_name.upper() and f'"{x["TABLE_NAME"].upper()}"' == table_name,
                all_table_columns,
            )
        )
        ...
        for name, properties_schema in self.flatten_schema.items():
            ...
            columns_to_replace.append(
                (safe_column_name(name), column_clause(name, properties_schema, is_iceberg_table))
            )
    ...
    def version_column(self, column_name, stream, is_iceberg_table=False):
        ...
        if is_iceberg_table:
            version_column = (
                f'ALTER ICEBERG TABLE {p_table_name} RENAME COLUMN {column_name} TO "{p_column_name}_{p_ver_time}"'
            )
        else:
    ...
    def sync_table(self):
        ...
        if self.table_cache:
            found_tables = list(
                filter(
                    lambda x: (
                        x["SCHEMA_NAME"] == self.schema_name.upper() and f'"{x["TABLE_NAME"].upper()}"' == table_name
                    ),
                    self.table_cache,
                )
            )
    ...
    def _refresh_table_pks(self):
        ...
        self.logger.debug(
            "Table: %s, Current PKs: %s | New PKs: %s ", self.stream_schema_message["stream"], current_pks, new_pks
        )
